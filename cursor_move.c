...
/*   Updated: 2019/12/07 13:47:30 by aait-ihi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cursor_move(t_ft_select *ft_select, int old_cursor_position)
{
	...
	line = old_cursor_position % ft_select->rows_count;
	col = (ft_select->max_len + 1) * (old_cursor_position / ft_select->rows_count);
	...
	line = ft_select->cursor % ft_select->rows_count;
	col = (ft_select->max_len + 1) * (ft_select->cursor / ft_select->rows_count);
	tputs(tgoto(tgetstr("cm", 0), col, line), 0, output);
